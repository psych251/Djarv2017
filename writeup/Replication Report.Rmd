...
df.data <- read.csv("../data/results.csv", header=FALSE, skip=17)
# add column names
colnames(df.data) <- c("reception_time", "ip", "controller", "order", "inner_element", "label", "latin_square", "legend","answer","correct","rt")


#### Data exclusion / filtering

# not sure about whether ``latin_square'' is needed or not
df.data.tidy <- df.data |>
  select("reception_time", "order", "label", "answer", "rt") |>
  ...
  group_by(order) |> # group by trial
  mutate(total_rt = sum(rt)) |>
  slice(1) |>
  ungroup() |>
  select("reception_time", "order", "answer", "label")


#### Prepare data for analysis - create columns etc.
df.data.clean <- df.data.tidy |>
  mutate(condition = str_extract_all(label, "[A-Z]+"))

# need to get the condition (predicate type, syntactic category, and block name: YES/NO)
# need to change the answers to numerical ratings
```
...
```{r}
# model = lmer(rating ~ predicate_type * answer_type + (predicate_type * answer_type  | participant) + (predicate_type * answer_type | item))
```
